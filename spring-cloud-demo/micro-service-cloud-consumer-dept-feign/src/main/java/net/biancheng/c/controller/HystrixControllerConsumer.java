...
public class HystrixControllerConsumer {
    ...
    // http://localhost:8000/consumer/hystrix/circuit/1
    // http://localhost:8000/consumer/hystrix/circuit/-2
    @RequestMapping(value = "/consumer/hystrix/circuit/{id}")
    public String deptCircuitBreaker(@PathVariable("id") Integer id){
        String result = deptHystrixService.deptCircuitBreaker(id);
